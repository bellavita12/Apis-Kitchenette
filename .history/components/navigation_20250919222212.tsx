...
export function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const heroHeight = window.innerHeight - 80 // adjust if needed
        ...
        {isOpen && (
          <div
            className={`md:hidden border-t animate-fade-in-up ${
              scrolled ? "bg-white text-gray-800" : "bg-black/90 text-white"
            }`}
          >
            <div className="px-4 py-4 space-y-4">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="block hover:text-primary transition-colors duration-200 font-medium"
