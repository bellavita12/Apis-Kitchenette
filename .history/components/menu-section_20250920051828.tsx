      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12 animate-fade-in-up animate-delay-200">
          {categories.map((category) => (
            <Button
              ...
              className={`text-lg px-6 py-3 hover-lift whitespace-nowrap transition-all duration-300 ${
                activeCategory === category.id
                  ? "bg-[#822527] text-white shadow-md"
                  : "hover:bg-[#822527]/10 hover:text-[#822527]"
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.map((item, index) => (
            <Card
              key={item.name}
              className="group hover:shadow-2xl hover:-translate-y-2 hover:scale-[1.03] transition-transform duration-300 border-border bg-card overflow-hidden"
            ...
            >
              <div className="relative overflow-hidden">
                ...
                />
                {item.popular && (
                  <Badge className="absolute top-4 left-4 bg-[#F3992C] text-white animate-bounce-in">
                    Popular Choice
                  </Badge>
                )}
              </div>
              ...
              <CardContent className="p-6">
                <h3 className="text-xl font-bold mb-2 font-[var(--font-space-grotesk)] group-hover:text-[#822527] transition-colors duration-300">
                  {item.name}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {item.dietary.map((diet) => (
                    <Badge key={diet} variant="outline" className="text-xs border-[#822527]/20 text-[#822527]">
                      {diet}
                    </Badge>
                  ))}
                </div>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-lg font-semibold text-[#F3992C]">For more details</span>
                  <Button
                    size="sm"
                    className="bg-[#822527] hover:bg-[#F3992C] text-white transition-colors duration-300 hover-glow"
                    onClick={() =>
        ...
        <div className="text-center mt-12 animate-fade-in-up animate-delay-400">
          ...
          <Button
            size="lg"
            className="bg-[#822527] text-white border-2 border-[#822527] hover:bg-[#F3992C] hover:text-white transition-colors duration-300 hover-glow"
            onClick={() =>
              openWhatsApp(
